...
export class InstanceRepository implements InstanceProtocol {
    ...
    async findInstanceById(id: string): Promise<InstanceEntity | null> {
    ...
    async updateInstanceById(id: string, data: Prisma.InstanceUpdateInput): Promise<InstanceEntity> {
    ...
    async deleteInstanceById(id: string): Promise<InstanceEntity> {
        return await this.prisma.instance.delete({ where: { id } });
    }

    async findActiveInstances(): Promise<{ id: string; userId: string; sessionStorage: string }[]> {
