...
export class InstanceService {
  ...
  async createInstance(
    ...
    try {
      ...
      const existingInstance = await this.instanceRepo.findInstanceByName(
        createInstanceDto.name,
      );
      if (existingInstance) {
        throw new ConflictException(
          `Instance with name '${createInstanceDto.name}' already exists`,
        );
      }
  ...
  private validateCreateInstanceDto(dto: CreateInstanceDto): void {
    ...
    if (
      !dto.businessName ||
      typeof dto.businessName !== 'string' ||
    ...
    if (
      !dto.businessWhatsAppNo ||
      typeof dto.businessWhatsAppNo !== 'string' ||
      !/^\+?[1-9]\d{1,14}$/.test(dto.businessWhatsAppNo)
    ) {
      errors.push(
        'Business WhatsApp number is required and must be a valid phone number',
      );
    }

    if (
      !dto.businessCountry ||
      typeof dto.businessCountry !== 'string' ||
      dto.businessCountry.trim() === ''
    ) {
      errors.push(
        'Business country is required and must be a non-empty string',
      );
    }

    if (
      !dto.environment ||
      typeof dto.environment !== 'string' ||
      !['production', 'development'].includes(dto.environment.toLowerCase())
    ) {
      errors.push(
        'Environment is required and must be either "PRODUCTION", "DEVELOPMENT"',
      );
    }

    if (
      !dto.sessionStorage ||
      typeof dto.sessionStorage !== 'string' ||
      !['redis', 'mongodb', 'postgreql'].includes(
        dto.sessionStorage.toLowerCase(),
      )
    ) {
      errors.push(
        'Session storage is required and must be either "REDIS", "POSTGRESQL" or "MONGODB"',
      );
    }
  ...
  async updateInstance(
    ...
    try {
      ...
      const updatedInstance = await this.instanceRepo.updateInstanceById(
        id,
        updateInstanceDto,
      );
  ...
  async toggleInstanceActive(
    ...
    const updatedInstance = await this.instanceRepo.updateInstanceById(id, {
