import { Injectable, StreamableFile, BadRequestException, NotFoundException, ForbiddenException } from '@nestjs/common';
...
export class FileService {
    ...
    async saveFile(file: Express.Multer.File, userId: string): Promise<string> {
        ...
        if (file.size > maxSizeBytes) {
            throw new BadRequestException(`File size exceeds the maximum limit of ${maxSizeMB}MB`);
        }
    ...
    async deleteFile(fileName: string, userId: string): Promise<void> {
        ...
        const fileIndex = this.fileMetadata.findIndex(file => file.fileName === fileName && file.userId === userId);
        if (fileIndex === -1) {
            throw new NotFoundException('File not found');
        }
    ...
    private async processAndSaveFile(file: Express.Multer.File, userDir: string, userId: string): Promise<string> {
    ...
    async getAllUserDirectories(): Promise<string[]> {
        const uploadDir = this.configService.get<string>('UPLOAD_DIR', './uploads');
        try {
            const directories = await fsPromises.readdir(uploadDir, { withFileTypes: true });
            return directories
    ...
    async getFilesByUserId(userId: string): Promise<FileMetadata[]> {
        ...
        try {
            ...
            return files.map(fileName => {
                const metadata = this.fileMetadata.find(fm => fm.fileName === fileName && fm.userId === userId);
                return metadata || {
                    fileName,
