...
export class AuthService {
  ...
  async registerNewUser(
    ...
    try {
      ...
      const newUser = await this.userDBRepo.createUser(
        userDataWithHashedPassword,
      );
      this.logger.log(
        `User registered successfully: ${newUser.username}`,
        'AuthService',
      );
  ...
  private async checkExistingUser(registrationData: UserRegistrationDto) {
    const isEmailExist = await this.userDBRepo.findUserByEmail(
      registrationData.emailAddress.toLowerCase().trim(),
    );
    if (isEmailExist) {
      throw new ConflictException('Email already exists');
    }

    const isUserNameExist = await this.userDBRepo.findUserByUserName(
    ...
    const isPhoneNumberExist = await this.userDBRepo.findUserByPhoneNumber(
  ...
  private async findUserByEmailOrUsername(
    ...
    if (!user) {
      throw new BadRequestException(
        `${isEmail ? 'Email' : 'Username'} not found`,
      );
    }
  ...
  private async setJwtTokentoBrowser(
    ...
    try {
      ...
      res.setHeader(
        'Set-Cookie',
        `authToken=${accessToken}; Path=/; HttpOnly; Secure; SameSite=Strict`,
      );
      // Set JWT as HttpOnly, Secure, and SameSite cookie in the response
      res.cookie('authToken', accessToken, {
        httpOnly: true,
        secure:
          this.env.get<string>('NODE_ENV') === 'production' ? true : false,
        sameSite:
          this.env.get<string>('NODE_ENV') === 'production' ? 'strict' : 'none', // Or 'Strict' or 'None' depending on your needs
      ...
      this.logger.error(
        `Failed to set JWT token: ${error.message}`,
        error.stack,
        'AuthService',
      );
  ...
  async chnageUserPassword(
    newPassData: forgotPasswordDto,
    res: ExpressResponse,
  ) {
    ...
    const isUserValid = await this.userDBRepo.findUserByUserName(
      newPassData.username,
    );
    ...
    return await this.userDBRepo.updateUserById(isUserValid.id, {
      password: await this.hashPassword(newPassData.newPassword),
    });
  }
