import { Controller, Post, Body, Param, UseGuards, HttpException, HttpStatus, ValidationPipe, UsePipes } from '@nestjs/common';
import { JwtAuthGuard } from '../Auth/jwt-auth-guard';
import { MessagingService } from './services/message-service';
import { ApiTags, ApiOperation, ApiResponse, ApiBearerAuth } from '@nestjs/swagger';
import { SendButtonMessageDto, SendContactMessageDto, SendListMessageDto, SendLocationMessageDto, SendMediaMessageDto, SendTextMessageDto } from './types/message-types';
...
export class MessagingController {
    ...
    async sendTextMessage(
        ...
        try {
            const success = await this.messagingService.sendTextMessage(instanceId, body.recipient, body.message);
            if (!success) {
                throw new HttpException('Failed to send text message', HttpStatus.INTERNAL_SERVER_ERROR);
            }
    ...
    async sendMediaMessage(
        ...
        try {
            ...
            if (!success) {
                throw new HttpException('Failed to send media message', HttpStatus.INTERNAL_SERVER_ERROR);
            }
    ...
    async sendButtonMessage(
        ...
        try {
            const success = await this.messagingService.sendButtonMessage(instanceId, body.recipient, body.content, body.buttons);
            if (!success) {
                throw new HttpException('Failed to send button message', HttpStatus.INTERNAL_SERVER_ERROR);
            }
            return { message: 'Button message sent successfully' };
        } catch (error) {
            this.logger.error(`Failed to send button message: ${error.message}`, error.stack);
            throw new HttpException(error.message, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async sendListMessage(
        ...
        try {
            ...
            this.logger.error(`Failed to send list message: ${error.message}`, error.stack);
            throw new HttpException(error.message, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async sendLocationMessage(
        ...
        try {
            ...
            if (!success) {
                throw new HttpException('Failed to send location message', HttpStatus.INTERNAL_SERVER_ERROR);
            }
