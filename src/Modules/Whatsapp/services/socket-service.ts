...
export class SocketService implements OnModuleInit {
  ...
  private async initializeActiveSockets(): Promise<void> {
    try {
      ...
      this.logger.log(
        `Found ${activeInstances.length} active instances. Initializing sockets...`,
      );
      ...
      for (const instance of activeInstances) {
        try {
          ...
          this.logger.error(
            `Failed to initialize socket for instance ${instance.id}`,
            error,
          );
        }
        await delay(1000);
  ...
  async getOrCreateSocket(instanceId: string): Promise<WASocket> {
    ...
    try {
      ...
      if (error instanceof Boom) {
        throw new Error(
          `Failed to create socket: ${error.output?.payload?.message || error.message}`,
        );
      }
      throw error;
    }
  ...
  async makeSocket(
    ...
    if (
      existingSocket &&
      existingSocket?.ws.readyState === existingSocket?.ws.OPEN
    ) {
      this.logger.log(
        `Socket already exists and is open for instance ${instanceId}`,
      );
    ...
    const { state, saveCreds } = await this.getInstanceSessionStorage({
      id: instanceId,
      sessionStorage,
    });
  ...
  private setupSocketListeners(
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (isNewLogin) {
        this.eventEmitter.emit(`qr.update.${instanceId}`, {
          data: 'Connected!',
        });
      ...
      if (connection === 'open') {
        await this.instanceDB.updateInstanceById(instanceId, {
          isActive: true,
        });
      ...
      if (
        (lastDisconnect?.error as Boom)?.message.toLowerCase() ===
        'user choice!'
      ) {
        return;
      }
      ...
      if (!shouldReconnect) {
        this.eventEmitter.removeAllListeners(`qr.update.${instanceId}`);
        this.logger.log(
          `Stopping reconnection attempts for instance ${instanceId}`,
        );
        this.socketMap.delete(instanceId);
    ...
    socket.ev.on('creds.update', () => {
      saveCreds().catch((error) =>
        this.logger.error(
          `Failed to save credentials for instance ${instanceId}${error}`,
        ),
      );
    });
    ...
    socket.ev.on('groups.update', (groupMetadata) => {
      groupMetadata.forEach((metadata) => {
        const cachedMetadata = this.groupMetadataCache.get<GroupMetadata>(
          metadata.id,
        );
        if (cachedMetadata) {
  ...
  async generatePairingCode(
    ...
    try {
      ...
      await this.deleteSession(instanceId);
      this.logger.error(
        `Failed to generate pairing code for instance ${instanceId}`,
        error,
      );
  ...
  private async immediateReconnect(
    userId: string,
    sessionStorage: string,
  ): Promise<void> {
  ...
  private async reconnectWithBackoff(
    ...
    const attemptCount =
      (this.msgRetryCounterCache.get('reconnectAttempts') as number) || 0;
    const delay = this.calculateBackoffDelay(attemptCount);

    this.logger.log(
      `Reconnecting for user ${userId} in ${delay}ms (attempt ${attemptCount + 1})`,
    );
  ...
  private async getInstanceSessionStorage(instance: {
    id: string;
    sessionStorage: string;
    ...
    const handler =
      storageHandlers[instance.sessionStorage] || storageHandlers.default;

    try {
      return await handler();
    } catch (error) {
      this.logger.error(
        `Failed to get auth state for ${instance.sessionStorage}`,
        error,
      );
      this.logger.log(
        `Falling back to MultiFileAuth for session: ${sessionName}`,
      );
      return storageHandlers.default();
    }
  }

  private async getRedisAuthState(
    sessionName: string,
  ): Promise<{ state: AuthenticationState; saveCreds: () => Promise<void> }> {
    try {
  ...
  async loggedOutEndSocket(instanceId: string): Promise<void> {
    try {
      ...
      this.logger.log(
        `Logged out end socket process completed for instance ${instanceId}`,
      );
    } catch (error) {
      this.logger.error(
        `Error in loggedOutEndSocket for instance ${instanceId}`,
        error,
      );
      throw error;
    }
  ...
  private async getPostgresAuthState(
    sessionName: string,
  ): Promise<{ state: AuthenticationState; saveCreds: () => Promise<void> }> {
    return await usePostgreSQLAuthState(this.pgPool, sessionName);
  }

  private async getMongoAuthState(
    sessionName: string,
  ): Promise<{ state: AuthenticationState; saveCreds: () => Promise<void> }> {
    ...
    const client = this.mongoDb
      .getConnectedClient()
      .db(this.configService.get<string>('SESSION_STORAGE_NAME', 'default'))
      .collection<AuthDocument>(sessionName);
  ...
  private async deleteSession(instanceID: string): Promise<void> {
    ...
    const handler =
      deletionHandlers[users.sessionStorage] || deletionHandlers.default;
    await handler();
  }

  private async deleteRedisSession(sessionName: string): Promise<void> {
    const { host, port, password } = this.getRedisConfig();
    const { deleteSession } = await useRedisAuthState(
      { host, port, password },
      sessionName,
    );
    await deleteSession();
  ...
  private async deletePostgresSession(sessionName: string): Promise<void> {
    const { deleteSession } = await usePostgreSQLAuthState(
      this.pgPool,
      sessionName,
    );
    await deleteSession();
  ...
  private async deleteMultiFileSession(sessionName: string): Promise<void> {
    ...
    try {
      await fs.rm(path.join(process.cwd(), sessionName), {
        recursive: true,
        force: true,
      });
    } catch (error) {
      this.logger.error(
        `Failed to delete multi-file session: ${sessionName}`,
        error,
      );
    }
  }
